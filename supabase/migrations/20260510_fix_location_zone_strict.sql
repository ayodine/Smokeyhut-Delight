-- Fix get_product_stats_lists: use delivery_zone strictly, exclude pickup/walk-in entries.
-- Removes the address-parse fallback that was surfacing "walkin", "pick", "manual/offline" etc.
...
AS $function$
  ...
  items AS (
    SELECT oi.product_id, oi.name AS item_name, oi.qty, oi.price,
           co.delivery_zone, co.customer_phone
    FROM order_items oi
  ...
  by_location AS (
    SELECT
      TRIM(delivery_zone) AS name,
      COUNT(*)::numeric   AS value
    FROM confirmed_orders
    WHERE delivery_zone IS NOT NULL
      AND TRIM(delivery_zone) <> ''
      AND LOWER(TRIM(delivery_zone)) NOT IN (
        'store pickup', 'pickup', 'walkin', 'walk-in', 'walk in',
        'manual/offline', 'manual/pickup', 'manual / offline',
        'pick', 'in-store', 'in store', 'offline'
      )
    GROUP BY TRIM(delivery_zone)
